...
public interface TRAN_MAIN_TRM_WRK_REP extends JpaRepository<TRAN_MAIN_TRM_WRK_ENTITY, String> {
	...
	@Query(value = "SELECT TRAN_DATE_BAL FROM BGLS_DAILY_ACCT_BAL WHERE ACCT_NUM = :accountNum " + " AND TRAN_DATE = ( "
			+ "          SELECT MAX(TRAN_DATE) " + "          FROM BGLS_DAILY_ACCT_BAL "
			+ "          WHERE ACCT_NUM = :accountNum "
			+ "            AND TRAN_DATE < TO_DATE(:tranDate, 'DD-MM-YYYY') " + "      )", nativeQuery = true)
	List<BigDecimal> findLatestTRAN_DATE_BALByAccountNumberBatchJob(@Param("accountNum") String accountNum,
	...
	@Query(value = "SELECT TRAN_DATE_BAL " + "FROM BGLS_DAILY_ACCT_BAL " + "WHERE ACCT_NUM = :accountNum "
			+ "AND TRAN_DATE = ( " + "    SELECT MAX(TRAN_DATE) " + "    FROM BGLS_DAILY_ACCT_BAL "
			+ "    WHERE ACCT_NUM = :accountNum " + "    AND TRAN_DATE < :endDate " + ")", nativeQuery = true)
	...
	@Query(value = "SELECT TRAN_DATE_BAL FROM BGLS_DAILY_ACCT_BAL WHERE ACCT_NUM = :accountNum " + " AND TRAN_DATE = ( "
			...
			+ "          WHERE ACCT_NUM = :accountNum "
			+ "            AND TRAN_DATE < TO_DATE(:tranDate, 'DD-MM-YYYY') " + "      )", nativeQuery = true)
	BigDecimal getTrandateBalBatchJob(@Param("accountNum") String accountNum, @Param("tranDate") String tranDate);
	...
	@Modifying
	@Transactional
	@Query(value = " UPDATE BGLS_DAILY_ACCT_BAL   " + " SET   " + "     GL_CODE        = :glCode,  "
			+ "     GL_DESC        = :glDesc,  " + "     GLSH_CODE      = :glshCode,  "
			+ "     GLSH_DESC      = :glshDesc,  " + "     ACCT_NAME      = :acctName,  "
			+ "     ACCT_CRNCY     = :acctCrncy,  " + "     TRAN_DR_BAL    = :totalDebit,  "
			+ "     TRAN_CR_BAL    = :totalCredit,  " + "     TRAN_DATE_BAL  = :tranDateBal,  "
			+ "     TRAN_TOT_NET   = :netAmount,  " + "     END_TRAN_DATE  = TO_DATE('2099-12-31', 'YYYY-MM-DD'),  "
			+ "     ENTRY_USER_ID  = 'SYSTEM',  " + "     ENTRY_TIME     = SYSDATE,  " + "     DEL_FLG        = 'N',  "
			+ "     OPENING_BAL    = :updatedOpenBal  " + " WHERE   " + "     ACCT_NUM  = :accountNum  "
			+ "     AND TRAN_DATE = TO_DATE(:tranDate, 'DD-MM-YYYY') ", nativeQuery = true)
	void UpdateExsistAccountBalanceBatchJob(@Param("glCode") String glCode, @Param("glDesc") String glDesc,
			@Param("glshCode") String glshCode, @Param("glshDesc") String glshDesc,
			@Param("accountNum") String accountNum, @Param("acctName") String acctName,
			@Param("acctCrncy") String acctCrncy, @Param("tranDateBal") BigDecimal tranDateBal,
			@Param("tranDate") String tranDate, // Corrected parameter name
	...
	@Query(value = "SELECT acct_num, " + "acct_name, " + "COUNT(TRAN_AMT) AS COUNTAMT, "
			...
			+ "SUM(CASE WHEN PART_TRAN_TYPE = 'Debit' THEN TRAN_AMT ELSE 0 END) AS TOTAL_DEBIT, "
			+ "SUM(CASE WHEN PART_TRAN_TYPE = 'Credit' THEN TRAN_AMT "
			+ "         WHEN PART_TRAN_TYPE = 'Debit' THEN -TRAN_AMT " + "ELSE 0 END) AS NETAMT,TRAN_DATE "
			+ "FROM BGLS_TRM_WRK_TRANSACTIONS " + "WHERE DEL_FLG != 'Y' " + "AND TRAN_STATUS = 'POSTED' "
			+ "AND TRUNC(TRAN_DATE) BETWEEN TO_DATE( :from_date, 'DD-MM-YYYY') AND TO_DATE(:to_date, 'DD-MM-YYYY') AND ACCT_NUM = :acct_num "
			+ "GROUP BY acct_num, acct_name,TRAN_DATE", nativeQuery = true)
	List<Object[]> getNetDebitCreditWithCountForCurrentDateByAcctNum(@Param("from_date") String from_date,
			@Param("to_date") String to_date, @Param("acct_num") String acct_num);
...
//	List<TRAN_MAIN_TRM_WRK_ENTITY> getrecoveryDetailsvalue();
	...
	@Query(value = "SELECT * FROM ( " + "SELECT * FROM bgls_trm_wrk_transactions "
			+ "WHERE flow_code = 'INDEM' AND TRAN_PARTICULAR = 'Interest Applied' "
			+ ") WHERE ROWNUM <= 200", nativeQuery = true)
	List<TRAN_MAIN_TRM_WRK_ENTITY> getInterestDetailsValue();
...
//					                                                   @Param("flowCode") String flowCode);
	...
	// Normal flow queries
	@Query(value = "SELECT * FROM bgls_trm_wrk_transactions "
			+ "WHERE flow_code = :flowCode AND LOWER(acct_num) LIKE '%' || :keyword || '%'", nativeQuery = true)
	List<TRAN_MAIN_TRM_WRK_ENTITY> searchByAccountNumber(@Param("keyword") String keyword,
			@Param("flowCode") String flowCode);

	@Query(value = "SELECT * FROM bgls_trm_wrk_transactions "
			+ "WHERE flow_code = :flowCode AND LOWER(acct_name) LIKE '%' || :keyword || '%'", nativeQuery = true)
	List<TRAN_MAIN_TRM_WRK_ENTITY> searchByAccountName(@Param("keyword") String keyword,
			@Param("flowCode") String flowCode);
	...
	@Query(value = "SELECT * FROM bgls_trm_wrk_transactions "
			...
			+ "     :filterType = 'Account Name' AND LOWER(acct_name) LIKE '%' || :keyword || '%') ", nativeQuery = true)
	List<TRAN_MAIN_TRM_WRK_ENTITY> searchRecovery(@Param("keyword") String keyword,
			@Param("filterType") String filterType);

	// 🟩 Download all Disbursement (Loan Disbursed) records
	@Query(value = "SELECT * FROM bgls_trm_wrk_transactions "
			+ "WHERE flow_code = 'DISBT' AND TRAN_PARTICULAR = 'Loan Disbursed'", nativeQuery = true)
	List<TRAN_MAIN_TRM_WRK_ENTITY> downloadDisbursementDetails();

	// 🟩 Download all Interest Applied records
	@Query(value = "SELECT * FROM bgls_trm_wrk_transactions "
			+ "WHERE flow_code = 'INDEM' AND TRAN_PARTICULAR = 'Interest Applied'", nativeQuery = true)
	List<TRAN_MAIN_TRM_WRK_ENTITY> downloadInterestDetails();

	// 🟩 Download all Fee Charged records
	@Query(value = "SELECT * FROM bgls_trm_wrk_transactions "
			+ "WHERE flow_code = 'FEEDEM' AND TRAN_PARTICULAR = 'Fee Charged'", nativeQuery = true)
	List<TRAN_MAIN_TRM_WRK_ENTITY> downloadFeeDetails();

	// 🟩 Download all Penalty Charged records
	@Query(value = "SELECT * FROM bgls_trm_wrk_transactions "
			+ "WHERE flow_code = 'PENDEM' AND TRAN_PARTICULAR = 'Penalty Charged'", nativeQuery = true)
	List<TRAN_MAIN_TRM_WRK_ENTITY> downloadPenaltyDetails();

	// 🟩 Download all Recovery records (Principal, Interest, Fee, Penalty)
	@Query(value = "SELECT * FROM bgls_trm_wrk_transactions "
			+ "WHERE tran_particular IN ('Principal Recovery','Interest Recovery','Fee Recovery','Penalty Recovery')", nativeQuery = true)
	List<TRAN_MAIN_TRM_WRK_ENTITY> downloadRecoveryDetails();

	@Modifying
	@Transactional
	@Query(value = "CALL DAYEND_INTEREST_DEMAND_1(:ENTRY_USER)", nativeQuery = true)
	void runInterestDemand(@Param("ENTRY_USER") String ENTRY_USER);
	...
	@Modifying
	@Transactional
	@Query(value = "CALL DAYEND_PENALITY_1(:ENTRY_USER)", nativeQuery = true)
	void runPenalty(@Param("ENTRY_USER") String ENTRY_USER);
	...
	@Query(value = "SELECT * FROM BGLS_TRM_WRK_TRANSACTIONS " + "WHERE TRAN_STATUS = 'POSTED' "
	...
	@Query(value = "SELECT * FROM BGLS_TRM_WRK_TRANSACTIONS " + "ORDER BY TRAN_ID "
			+ "OFFSET ?1 ROWS FETCH NEXT ?2 ROWS ONLY", nativeQuery = true)
	List<TRAN_MAIN_TRM_WRK_ENTITY> getLoanActWithtranid(int offset, int limit);

	@Query(value = "SELECT * FROM BGLS_TRM_WRK_TRANSACTIONS ORDER BY TRAN_ID OFFSET ?1 ROWS FETCH NEXT ?2 ROWS ONLY", nativeQuery = true)
	List<Object[]> getLoanActWithtranid1(int offset, int limit);

	@Query(value = "SELECT * FROM BGLS_TRM_WRK_TRANSACTIONS "
			+ "WHERE TRAN_ID =?1 ORDER BY TRAN_DATE, TRAN_ID, PART_TRAN_ID", nativeQuery = true)
	List<TRAN_MAIN_TRM_WRK_ENTITY> searchByTranIdLike(@Param("tranId") String tranId);

	@Query(value = "SELECT * FROM BGLS_TRM_WRK_TRANSACTIONS "
			+ "WHERE TRUNC(TRAN_DATE) = TO_DATE(:tranDate, 'DD-MM-YYYY') ORDER BY TRAN_DATE, TRAN_ID, PART_TRAN_ID", nativeQuery = true)
	List<TRAN_MAIN_TRM_WRK_ENTITY> searchByTranDate(@Param("tranDate") String tranDate);

	@Query(value = "SELECT * FROM BGLS_TRM_WRK_TRANSACTIONS "
			+ "WHERE ACCT_NUM =?1 ORDER BY TRAN_DATE, TRAN_ID, PART_TRAN_ID", nativeQuery = true)
	List<TRAN_MAIN_TRM_WRK_ENTITY> searchByAcctNumLike(@Param("acctNum") String acctNum);
