...
public interface HolidayMaster_Rep extends CrudRepository<HolidayMaster_Entity, Long> {
    ...
    @Query(value ="select CAL_YEAR,CAL_MONTH,to_char(RECORD_DATE,'dd-mm-yyyy')RECORD_DATE,TO_CHAR(RECORD_DATE, 'Day') AS DAY,HOLIDAY_DESC,HOLIDAY_REMARKS from HOLIDAY_MASTER WHERE CAL_YEAR= :year AND CAL_MONTH= :month",nativeQuery = true)
