...
//	@Query(value = "SELECT * FROM LOAN_ACCOUNT_MASTER_TBL", nativeQuery = true)
	...
	@Modifying
	@Transactional
	@Query(value = "INSERT INTO LOAN_ACCOUNT_MASTER_TBL_TEST_1 ( " + "    ENCODED_KEY, " + "    ID, "
			+ "    ACCOUNT_HOLDERTYPE, " + "    ACCOUNT_HOLDERKEY, " + "    CREATION_DATE, " + "    APPROVED_DATE, "
			+ "    LAST_MODIFIED_DATE, " + "    CLOSED_DATE, " + "    LAST_ACCOUNT_APPRAISALDATE, "
			+ "    ACCOUNT_STATE, " + "    ACCOUNT_SUBSTATE, " + "    PRODUCT_TYPEKEY, " + "    LOAN_NAME, "
			+ "    PAYMENT_METHOD, " + "    ASSIGNED_BRANCHKEY, " + "    LOAN_AMOUNT, " + "    INTEREST_RATE, "
			+ "    PENALTY_RATE, " + "    ACCRUED_INTEREST, " + "    ACCRUED_PENALTY, " + "    PRINCIPAL_DUE, "
			+ "    PRINCIPAL_PAID, " + "    PRINCIPAL_BALANCE, " + "    INTEREST_DUE, " + "    INTEREST_PAID, "
			+ "    INTEREST_BALANCE, " + "    INTEREST_FROMARREARSBALANCE, " + "    INTEREST_FROMARREARSDUE, "
			+ "    INTEREST_FROMARREARSPAID, " + "    FEES_DUE, " + "    FEES_PAID, " + "    FEES_BALANCE, "
			+ "    PENALTY_DUE, " + "    PENALTY_PAID, " + "    PENALTY_BALANCE, " + "    EXPECTED_DISBURSEMENTDATE, "
			+ "    DISBURSEMENT_DATE, " + "    FIRST_REPAYMENTDATE, " + "    GRACE_PERIOD, "
			+ "    REPAYMENT_INSTALLMENTS, " + "    REPAYMENT_PERIODCOUNT, " + "    DAYS_LATE, "
			+ "    DAYS_INARREARS, " + "    REPAYMENT_SCHEDULE_METHOD, " + "    CURRENCY_CODE, "
			+ "    SALE_PROCESSEDBYVGID, " + "    SALE_PROCESSEDFOR, " + "    SALE_REFERREDBY, "
			+ "    EMPLOYMENT_STATUS, " + "    JOB_TITLE, " + "    EMPLOYER_NAME, " + "    TUSCORE, "
			+ "    TUPROBABILITY, " + "    TUFULLNAME, " + "    TUREASON1, " + "    TUREASON2, " + "    TUREASON3, "
			+ "    TUREASON4, " + "    DISPOSABLE_INCOME, " + "    MANUALOVERRIDE_AMOUNT, "
			+ "    MANUALOVERRIDE_EXPIRY_DATE, " + "    CPFEES, " + "    DEPOSIT_AMOUNT, " + "    TOTAL_PRODUCT_PRICE, "
			+ "    RETAILER_NAME, " + "    RETAILER_BRANCH, " + "    VG_APPLICATION_ID, " + "    CONTRACT_SIGNED, "
			+ "    DATE_OF_FIRST_CALL, " + "    LAST_CALL_OUTCOME, " + "    ASONDATE, " + "    DISBURSEMENT_FLG, "
			+ "    INTEREST_FLG, " + "    FEES_FLG, " + "    RECOVERY_FLG " + ") " + "SELECT " + "    ENCODED_KEY, "
			+ "    ID, " + "    ACCOUNT_HOLDERTYPE, " + "    ACCOUNT_HOLDERKEY, "
			...
			+ "         THEN TO_DATE(LAST_ACCOUNT_APPRAISALDATE,'YYYY-MM-DD') ELSE NULL END, " + "    ACCOUNT_STATE, "
			+ "    ACCOUNT_SUBSTATE, " + "    PRODUCT_TYPEKEY, " + "    LOAN_NAME, " + "    PAYMENT_METHOD, "
			+ "    ASSIGNED_BRANCHKEY, "
			...
			+ "    REPAYMENT_SCHEDULE_METHOD, " + "    CURRENCY_CODE, " + "    SALE_PROCESSEDBYVGID, "
			+ "    SALE_PROCESSEDFOR, " + "    SALE_REFERREDBY, " + "    EMPLOYMENT_STATUS, " + "    JOB_TITLE, "
			+ "    EMPLOYER_NAME, "
			...
			+ "    TUFULLNAME, " + "    TUREASON1, " + "    TUREASON2, " + "    TUREASON3, " + "    TUREASON4, "
			...
			+ "         THEN TO_DATE(DATE_OF_FIRST_CALL,'YYYY-MM-DD') ELSE NULL END, " + "    LAST_CALL_OUTCOME, "
			+ "    CASE WHEN REGEXP_LIKE(ASONDATE,'^\\d{4}-\\d{2}-\\d{2}$') "
			+ "         THEN TO_DATE(ASONDATE,'YYYY-MM-DD') ELSE NULL END, " + "    'N', " + "    'N', " + "    'N', "
			+ "    'N' " + "FROM LOAN_ACCOUNT_MASTER_TBL_TEST", nativeQuery = true)
	...
	@Query(value = "WITH ZeroBalanceDue AS ( " + "    SELECT DUE_DATE AS LAST_ZERO_BAL_DATE "
			+ "    FROM LOAN_REPAYMENT_TBL " + "    WHERE PARENT_ACCOUNT_KEY = :encodedKey "
			+ "      AND (PRINCIPAL_EXP - PRINCIPAL_PAID = 0 " + "       AND INTEREST_EXP - INTEREST_PAID = 0 "
			+ "       AND FEE_EXP - FEE_PAID = 0 " + "       AND PENALTY_EXP - PENALTY_PAID = 0) "
			+ "      AND DUE_DATE < :tran_date " + "    ORDER BY DUE_DATE DESC " + "    FETCH FIRST 1 ROWS ONLY "
			+ "), " + "NextDueDate AS ( " + "    SELECT DUE_DATE AS NEXT_DUE_DATE " + "    FROM LOAN_REPAYMENT_TBL "
			+ "    WHERE PARENT_ACCOUNT_KEY = :encodedKey "
			+ "      AND DUE_DATE > (SELECT LAST_ZERO_BAL_DATE FROM ZeroBalanceDue) " + "    ORDER BY DUE_DATE ASC "
			+ "    FETCH FIRST 1 ROWS ONLY " + ") " + "SELECT " + "    a.CUSTOMER_ID, " + "    b.ENCODED_KEY, "
			+ "    b.employer_name, " + "    b.CREATION_DATE, " + "    b.ID, " + "    b.INTEREST_RATE, "
			+ "    b.DISBURSEMENT_DATE, " + "    b.REPAYMENT_INSTALLMENTS, " + "    b.LOAN_AMOUNT, "
			+ "    b.LOAN_NAME, " + "    b.CURRENCY_CODE, " + "    d.ACCT_BAL, " + "    zbd.LAST_ZERO_BAL_DATE, "
			+ "    (COALESCE(zbd.LAST_ZERO_BAL_DATE, :tran_date) + 1) AS NEXT_DAY, " + "    nd.NEXT_DUE_DATE, "
			...
			+ "    ) AS DEMAND_APPLY, " + "    SUM( " + "        CASE WHEN c.DUE_DATE <= :tran_date "
			...
			+ "        SUM(CASE WHEN c.DUE_DATE <= :tran_date " + "          AND (c.PRINCIPAL_EXP = c.PRINCIPAL_PAID) "
			+ "          AND (c.INTEREST_EXP = c.INTEREST_PAID) " + "          AND (c.FEE_EXP = c.FEE_PAID) "
			+ "        THEN (c.PRINCIPAL_PAID + c.INTEREST_PAID + c.FEE_PAID) ELSE 0 END) " + "    ) AS ARREARS_APPLY, "
			+ " a.FIRST_NAME || ' ' || a.LAST_NAME AS FIRST_NAME " + "FROM CLIENT_MASTER_TBL a "
			+ "JOIN LOAN_ACCOUNT_MASTER_TBL b ON a.ENCODED_KEY = b.ACCOUNT_HOLDERKEY "
			...
			+ "LEFT JOIN NextDueDate nd ON 1 = 1 " + "WHERE a.ENCODED_KEY = :holderKey " + "  AND b.ID = :id "
			+ "  AND (c.DUE_DATE <= :tran_date OR c.DUE_DATE IS NULL) " + "GROUP BY " + "    a.CUSTOMER_ID, "
			+ "    b.ENCODED_KEY, " + "    b.employer_name, " + "    b.CREATION_DATE, " + "    b.ID, "
			+ "    b.INTEREST_RATE, " + "    b.DISBURSEMENT_DATE, " + "    b.REPAYMENT_INSTALLMENTS, "
			+ "    b.LOAN_AMOUNT, " + "    b.LOAN_NAME, " + "    b.CURRENCY_CODE, " + "    d.ACCT_BAL, "
			+ "    zbd.LAST_ZERO_BAL_DATE, " + "    nd.NEXT_DUE_DATE,"
			+ "  a.FIRST_NAME || ' ' || a.LAST_NAME ", nativeQuery = true)
	Object[] getCustomer(@Param("tran_date") Date tran_date, @Param("holderKey") String holderKey,
			@Param("id") String id, @Param("encodedKey") String encodedKey);

	@Query(value = "SELECT a.ENCODED_KEY, " + "       b.DUE_DATE dueDate, " + "       b.REPAID_DATE repaidDate, "
			+ "       b.PRINCIPAL_EXP principalExp, " + "       b.PRINCIPAL_PAID principalPaid, "
			+ "       b.PRINCIPAL_DUE principalDue, " + "       b.INTEREST_EXP interestExp, "
			+ "       b.INTEREST_PAID interestPaid, " + "       b.INTEREST_DUE interestDue, "
			+ "       b.FEE_EXP feeExp, " + "       b.FEE_PAID feePaid, " + "       b.FEE_DUE feeDue, "
			+ "       b.PENALTY_EXP penaltyExp, " + "       b.PENALTY_PAID penaltyPaid, "
			+ "       b.PENALTY_DUE penaltyDue " + "FROM LOAN_ACCOUNT_MASTER_TBL a "
			+ "JOIN LOAN_REPAYMENT_TBL b ON a.ENCODED_KEY = b.PARENT_ACCOUNT_KEY " + "WHERE a.ENCODED_KEY = ?1 "
			+ "  AND a.DEL_FLG = 'N' " + "  AND b.DEL_FLG = 'N' " + "ORDER BY b.DUE_DATE ASC", nativeQuery = true)
...
//	List<LOAN_ACT_MST_ENTITY> getLoanActFilterVerified();
	...
	@Query(value = "SELECT id, loan_name, encoded_key, currency_code " + "FROM loan_account_master_tbl "
...
//	@Query(value = "SELECT l.*, c.MOBILE_PHONE AS mobile_phone,c.FIRST_NAME AS first_name,c.LAST_NAME AS last_name FROM LOAN_ACCOUNT_MASTER_TBL l INNER JOIN CLIENT_MASTER_TBL c ON c.ENCODED_KEY = l.ACCOUNT_HOLDERKEY ORDER BY l.ID  WHERE l.last_modified_date > l.approved_date ORDER BY id OFFSET ?1 ROWS FETCH NEXT ?2 ROWS ONLY", nativeQuery = true)
	@Query(value = "SELECT l.*, " + "c.MOBILE_PHONE AS mobile_phone, " + "c.FIRST_NAME AS first_name, "
			+ "c.LAST_NAME AS last_name " + "FROM LOAN_ACCOUNT_MASTER_TBL l "
			+ "INNER JOIN CLIENT_MASTER_TBL c ON c.ENCODED_KEY = l.ACCOUNT_HOLDERKEY "
			+ "WHERE l.last_modified_date > l.approved_date " + "ORDER BY l.ID "
			+ "OFFSET ?1 ROWS FETCH NEXT ?2 ROWS ONLY", nativeQuery = true)
...
//	@Query(value = "SELECT * FROM LOAN_ACCOUNT_MASTER_TBL WHERE last_modified_date < approved_date ORDER BY id OFFSET ?1 ROWS FETCH NEXT ?2 ROWS ONLY", nativeQuery = true)
	...
	@Query(value = " SELECT l.*, c.MOBILE_PHONE AS mobile_phone,c.FIRST_NAME AS first_name,c.LAST_NAME AS last_name    "
			...
			+ " WHERE LOWER(l.ID) LIKE  LOWER('%' || :loanId || '%')", nativeQuery = true)
	List<Object[]> searchByLoanIdLike(@Param("loanId") String loanId);

	@Query(value = " SELECT l.*, c.MOBILE_PHONE AS mobile_phone,c.FIRST_NAME AS first_name,c.LAST_NAME AS last_name    "
			+ " FROM LOAN_ACCOUNT_MASTER_TBL l " + " INNER JOIN CLIENT_MASTER_TBL c "
			+ "     ON c.ENCODED_KEY = l.ACCOUNT_HOLDERKEY "
			+ " WHERE LOWER(l.LOAN_NAME) LIKE  LOWER('%' || :loanType || '%')", nativeQuery = true)
	List<Object[]> searchByLoanTypeLike(@Param("loanType") String loanType);

	@Query(value = " SELECT l.*, c.MOBILE_PHONE AS mobile_phone,c.FIRST_NAME AS first_name,c.LAST_NAME AS last_name    "
			+ " FROM LOAN_ACCOUNT_MASTER_TBL l " + " INNER JOIN CLIENT_MASTER_TBL c "
			+ "     ON c.ENCODED_KEY = l.ACCOUNT_HOLDERKEY "
			+ " WHERE LOWER(c.MOBILE_PHONE) LIKE  LOWER('%' || :loanMobileNumber || '%')", nativeQuery = true)
	...
	@Query(value = " SELECT l.*, " + "        c.MOBILE_PHONE AS mobile_phone, "
			+ "        c.FIRST_NAME   AS first_name, " + "        c.LAST_NAME    AS last_name "
			+ " FROM LOAN_ACCOUNT_MASTER_TBL l " + " INNER JOIN CLIENT_MASTER_TBL c "
			+ "   ON c.ENCODED_KEY = l.ACCOUNT_HOLDERKEY " + " WHERE l.ACCOUNT_STATE = :status " + " ORDER BY l.ID " + // ✅
																														// required
																														// before
																														// FETCH
			" FETCH FIRST 2000 ROWS ONLY ", nativeQuery = true)
...
//List<LOAN_ACT_MST_ENTITY> getloanDetails();
	...
	@Query(value = "SELECT l.ID, c.FIRST_NAME || ' ' || c.LAST_NAME AS accountHolderName, "
			+ "c.MOBILE_PHONE AS mobile_phone, c.FIRST_NAME AS first_name, c.LAST_NAME AS last_name "
			+ "FROM LOAN_ACCOUNT_MASTER_TBL l "
			+ "INNER JOIN CLIENT_MASTER_TBL c ON c.ENCODED_KEY = l.ACCOUNT_HOLDERKEY "
			+ "ORDER BY l.ID", nativeQuery = true)
	List<Object[]> getLoanNo();
	...
	@Query(value = "SELECT a.id, b.FIRST_NAME || ' ' || b.LAST_NAME AS full_name " + "FROM LOAN_ACCOUNT_MASTER_TBL a "
